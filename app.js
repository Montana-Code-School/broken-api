...
app.get("/api/beers", (req, res) => {
  // req = request data on the outgoing call, res = response from the call
  Beer.find((err, beers) => {
    // Call find() mongoose function (http://mongoosejs.com/docs/queries.html) to find all beers in database
    if (err) {
      // if an error occured => print error stack
      throw err;
    }
...
app.get("/api/beers/:_id", (req, res) => {
  // The : in front of _id in the url represents a parameter which can take one different _id values for different beers
  Beer.findById(req.params._id, (err, beer) => {
    // Call findById() mongoose function to find beer in database
    if (err) {
      throw err;
    }
    res.json(beer);
...
app.post("/api/beers", (req, res) => {
  ...
  Beer.create(newBeer, (err, beer) => {
    // Call create() mongoose function to create in database
    if (err) {
      throw err;
    }
    res.json(beer);
...
app.put("/api/beers/:_id", (req, res) => {
  ...
  Beer.findOneAndUpdate(query, updatedBeer, {}, (err, beer) => {
    // Call create() mongoose function to create new beer in database
    if (err) {
      throw err;
    }
    res.json(beer);
